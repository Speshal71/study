...
void HeatMap::init_boundary(double val, int dir)
{
    ...
    #pragma omp parallel for private(i, j, k) shared(arr)
    for (k = k_lower; k < k_upper; ++k) {
        for (j = j_lower; j < j_upper; ++j) {
            for (i = i_lower; i < i_upper; ++i) {
                arr[k * nx * ny + j * nx + i] = val;
            }
        }
    }
}
...
void HeatMap::boundary_to_buff(double *buff, int axis)
{
    ...
    #pragma omp parallel for private(i, j, k) shared(arr, buff)
    for (k = k_lower; k < k_upper; ++k) {
        for (j = j_lower; j < j_upper; ++j) {
            for (i = i_lower; i < i_upper; ++i) {
                buff[(this->*index_func)(i, j, k)] =  arr[k * nx * ny + j * nx + i];
            }
        }
    }
}
...
void HeatMap::set_boundary(double *buff, int axis)
{
    ...
    for (k = k_lower; k < k_upper; ++k) {
        for (j = j_lower; j < j_upper; ++j) {
            for (i = i_lower; i < i_upper; ++i) {
                arr[k * nx * ny + j * nx + i] = buff[(this->*index_func)(i, j, k)];
            }
        }
    }
}
...
double HeatMap::approximate()
{
    ...
    int i, j, k;
    double *arr_cur = u_cur;
    double *arr_prev = u_prev; 

    #pragma omp parallel for shared(arr_cur, arr_prev) private(i, j, k) reduction(max:local_eps)
    for (k = 1; k < (nz - 1); ++k) {
        for (j = 1; j < (ny - 1); ++j) {
            for (i = 1; i < (nx - 1); ++i) {
                arr_cur[to_index(i, j, k)] = (
                    (arr_prev[to_index(i + 1, j, k)] + arr_prev[to_index(i - 1, j, k)]) / (hx * hx) +
                    (arr_prev[to_index(i, j + 1, k)] + arr_prev[to_index(i, j - 1, k)]) / (hy * hy) +
                    (arr_prev[to_index(i, j, k + 1)] + arr_prev[to_index(i, j, k - 1)]) / (hz * hz)
                ) / (
                    2 / (hx * hx) + 2 / (hy * hy) + 2 / (hz * hz)
                );
                local_eps = std::max(local_eps, std::abs(arr_cur[to_index(i, j, k)] - arr_prev[to_index(i, j, k)]));
            }
        }
    }
